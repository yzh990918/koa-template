...
const { ParameterException, NotFoundException } = require("../core/http-execption")
...
class UploadService {
  ...
  async uploadOneFile(payload) {
    ...
    // 最后返回的路径
    const returnPath = `${moment().format('YYYY-MM-DD')}/${fileName}.${extraName}`
  ...
  async destroyOneFile(id) {
    try {
      ...
      // 实际删除文件的路径
      const realPath = path.join(process.cwd(),uploadPath,currentAttachment.path)
  ...
  async getLoginedFilelist(payload) {
    ...
    try {
        ...
        let fileList = await attachment.find({creatorId: id},{ creatorId : 0}).skip(page * limit).limit(limit)
  ...
  async getSystemFileList(payload) {
    try {
      ...
      let fileList = await attachment.find().skip(page * limit).limit(limit)
      for(let file of fileList) {
        file.path = `${baseUrl.development}/${file.path}`
      }
      res = {
